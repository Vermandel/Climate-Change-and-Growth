...
%   Detailed explanation goes here
    ...
    if DUM_tax == 1
        ...
        fun = @(x) [
            ...
            - x(3)^(theta2-1) + (sigma/(theta1*theta2)*x(4)*x(2)^-gamma)
            ];
        ...
        fun = @(x) [
            % y    = (1 - d2*x^2 - d1*x - d0) * (y/y_k)^alpha;
            x(2) - (1 - d2*x(1)^2 - d1*x(1) - d0) * (x(2)/y_k)^alpha;
            % x = 4*sigma*(y^(1-gamma))/deltaX
            x(1) - 4*sigma*(x(2)^(1-gamma))/deltaX
        ];
        x0 = [ 800 3];
        options = optimoptions('fsolve', ...
            'Display', 'iter', ...
